...
-->
		...
		<div class="table-container">
			...
			<el-table ref="newProductTable" :data="list" style="width: 100%;" @selection-change="handleSelectionChange" v-loading="listLoading" border>
				...
				<el-table-column label="姓名/手机号" align="center" :render-header="renderheader">
				...
				<el-table-column label="累计佣金/打款佣金" align="center" :render-header="renderheader">
					<template slot-scope="scope">
						<div style="color: red;">{{scope.row.yjPrice}}</div>
						<div style="color: orange;">{{scope.row.yJtx}}</div>
					</template>
				</el-table-column>
				<el-table-column label="累计管理收益/打款管理收益" align="center" :render-header="renderheader">
					<template slot-scope="scope">
						<div style="color: red;">{{scope.row.jJprice}}</div>
						<div style="color: orange;">{{scope.row.jJtx}}</div>
					</template>
				</el-table-column>
				<el-table-column label="累计阶段奖金/打款阶段奖金" align="center" :render-header="renderheader">
					<template slot-scope="scope">
						<div style="color: red;">{{scope.row.jdprice}}</div>
				...
				<el-table-column label="注册时间/审核时间/过期时间" align="center" :render-header="renderheader">
					<template slot-scope="scope">
...
<script>
	import { fetchList, levelTopById, memberStatus } from '@/api/retailerManage';
		...
		methods: {
			...
			export2Excel() {
				...
				require.ensure([], () => {
					...
					const tHeader = ["粉丝", "姓名", "手机号", "等级", "累计佣金","打款佣金","累计管理收益","打款管理收益",
					"累计阶段奖金","打款阶段奖金","团队分销商人数","注册时间","审核时间","过期时间",]; // 导出的excel表头名信息
					const filterVal = [
						"nickname",
						"nickname",
						"phone",
					...
					export_json_to_excel(tHeader, data, "分销商信息汇总"); // 导出的表格名称，根据需要自己命名
				});
			},
			//格式转换，直接复制即可,不需要修改什么
