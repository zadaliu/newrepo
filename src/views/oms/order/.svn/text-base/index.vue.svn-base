<template>
	<div class="app-container">
			...
			<el-table ref="orderTable" :data="list" style="width: 100%;" @selection-change="handleSelectionChange" v-loading="listLoading" border>
				...
				<el-table-column label="操作" width="200" align="center">
					<template slot-scope="scope">
						...
						<el-button size="mini"  type="danger" @click="handleCloseOrder(scope.$index, scope.row)" v-show="scope.row.status===0">删除订单</el-button>
		...
		<!--<logistics-dialog :id="id" v-model="logisticsDialogVisible"></logistics-dialog>-->
		<el-dialog title="订单追踪" :visible.sync="logisticsDialogVisible" width="30%">

			<div v-for="entity in  logisticsList">
				...
				<el-row type="flex" class="row-bg">
					...
					<el-col :span="6">
						<div class="grid-content bg-purple"> <span><img :src="entity.img"> </span></div>
					</el-col>
					<el-col :span="9" style="margin: auto;">
						
							<span style="text-align: center;">
								{{entity.expName+"  "+ entity.numbers}}</span>
				...
				<!--<el-steps direction="vertical" finish-status="success" space="50px">
					<el-step v-for="item in entity.list" :key="item.status" :title="item.status" :description="item.time"></el-step>
				</el-steps>-->
				
				
				 <el-timeline space="50px" :reverse="reverse">
				    <el-timeline-item
...
<script>
	import { fetchList, closeOrder, deleteOrder, goodsIntoByOrderId } from '@/api/order';
	...
	export default {
		...
		methods: {
			...
			handleViewLogistics(index, row) {
				this.id = row.id
				goodsIntoByOrderId(this.id).then(response => {
