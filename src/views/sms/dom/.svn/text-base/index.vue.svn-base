<template>
	<div class="app-container">
		...
		<el-card class="operate-container" shadow="never">
			...
			<span>数据列表</span>
			<el-button size="mini" class="btn-add" @click="handleAdd() ">添加弹窗广告</el-button>
		</el-card>
			...
			<el-table ref="homeAdvertiseTable " :data="list " style="width: 100%; " @selection-change="handleSelectionChange " v-loading="listLoading " border>
				...
				<el-table-column label="图片" align="center ">
					<template slot-scope="scope ">
						...
						<img style="height: 80px" :src="scope.row.domImg">
					</template>
...
<script>
	import { deleteSelectAll, fetchList, getTitle, updateTitle } from '@/api/dom';
	...
	export default {
		...
		methods: {
			...
			handleAdd() {
				...
				this.$router.push({
					path: '/sms/addDomManage'
				})
			},
			handleUpdate(index, row) {
				this.$router.push({
					path: '/sms/updateDomManage',
					query: {
						id: row.id
					}
				})
