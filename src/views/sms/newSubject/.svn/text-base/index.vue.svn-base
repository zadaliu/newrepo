		...
		<div class="table-container ">
			<el-table ref="homeAdvertiseTable " :data="list " style="width: 100%; " @selection-change="handleSelectionChange " v-loading="listLoading " border>
				...
				<el-table-column label="是否显示" align="center ">
					<template slot-scope="scope ">{{scope.row.showStatus|flagFilter}}</template>
				</el-table-column>
...
<script>
	import { deleteSelectAll, fetchList, getTitle, updateTitle } from '@/api/newSubject';
	...
	export default {
		...
		methods: {
			...
			handleAdd() {
				...
				this.$router.push({
					path: '/sms/addNewSubject'
				})
			},
			handleUpdate(index, row) {
				this.$router.push({
					path: '/sms/updateNewSubject',
					query: {
						id: row.id
					}
				})
			...
			deleteAdminSelectAll(ids) {
				this.$confirm('是否要删除该标签?', '提示', {
					...
					deleteSelectAll(params).then(response => {
						...
						// this.listQuery.pageNum= totalPage > 0 ? totalPage : 1;
            			this.listQuery.pageNum=1;
						this.getList();
						this.$message({
							type: 'success',
