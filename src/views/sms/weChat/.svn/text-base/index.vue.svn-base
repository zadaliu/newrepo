<template>
	<div class="app-container">
		...
		<el-card class="operate-container" shadow="never">
			...
			<span>数据列表</span>
			<el-button size="mini" class="btn-add" @click="handleAdd() ">添加WeChatBanner</el-button>
		</el-card>
		<div class="table-container ">
			<el-table ref="homeAdvertiseTable " :data="list " style="width: 100%; " @selection-change="handleSelectionChange " v-loading="listLoading " border>
				...
				<el-table-column label="排序编码 " align="center ">
					<template slot-scope="scope ">{{scope.row.numOrder}}</template>
				</el-table-column>
...
<script>
	import { deleteSelectAll, fetchList, getTitle, updateTitle } from '@/api/weChatBanner';
	...
	export default {
		...
		methods: {
			...
			handleAdd() {
				...
				this.$router.push({
					path: '/sms/addWeChatBannerManage'
				})
			},
			handleUpdate(index, row) {
				this.$router.push({
					path: '/sms/updateaWeChatBnnerManage',
					query: {
						id: row.id
					}
				})
