      ...
      至
	    <el-date-picker
          ...
          <el-form-item label="客户名称：">
            <el-input v-model="listQuery.name" class="input-width" placeholder="商品名称"></el-input>
          </el-form-item>
    ...
    <div class="table-container">
      <el-table ref="newProductTable"
        ...
        <el-table-column label="客户"   align="center">
          ...
          <span @click="handleToMember(scope.$index, scope.row)" >
          {{scope.row.memberNickname}}
          </span>
          </template>
        </el-table-column>
        <el-table-column label="领劵时间" align="center">
          <template slot-scope="scope">{{scope.row.createTime|formatDate}}</template>
        </el-table-column>
        <el-table-column label="用劵时间" align="center">
          <template slot-scope="scope">{{scope.row.useTime|formatDate}}</template>
        </el-table-column>
        <el-table-column label="领取来源" align="center">
          <template slot-scope="scope">{{scope.row.source| formatStatus}}</template>
        </el-table-column>
       
        
        <el-table-column label="订单详情" width="180" align="center">
          <template slot-scope="scope">
            <el-button size="mini"
                       type="text"
                       @click="handleToOrder(scope.$index, scope.row)">查看订单
            </el-button>
          </template>
        </el-table-column>
      </el-table>
...
<script>
  import {historyList } from '@/api/newcoupon';
  ...
  export default {
    ...
    methods: {
      ...
      handleToMember(index,row){
                this.$router.push({path: '/translate/translateUpdate', query: {id: row.memberId}})
      },
      handleToOrder(index,row){
                this.$router.push({path: '/oms/order', query: {id: row.id}})
      },
