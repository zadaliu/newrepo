<template>
	<div class="app-container">
		...
		<el-card class="operate-container" shadow="never">
			...
			<span>数据列表</span>
			<el-button size="mini" class="btn-add" @click="handleAdd() ">添加logo</el-button>
		</el-card>
...
<script>
	import { deleteSelectAll, fetchList, getTitle, updateTitle } from '@/api/logo';
	...
	export default {
		...
		methods: {
			...
			handleAdd() {
				...
				this.$router.push({
					path: '/sms/addLogoManage'
				})
			},
			handleUpdate(index, row) {
				this.$router.push({
					path: '/sms/updateLogoManage',
					query: {
						id: row.id
					}
				})
