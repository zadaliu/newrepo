<template>
	<div class="shop-transcity-add-or-update">
		<el-dialog :modal="false" title="选择配送区域" :close-on-click-modal="false" :visible.sync="visible">
			<el-form :model="dataForm" ref="dataForm" @keyup.enter.native="dataFormSubmit()" label-width="80px" style="height:400px">
				<el-scrollbar style="height:100%">
					<el-form-item size="mini" label="配送区域">
						<el-tree :data="menuList" :props="menuListTreeProps" node-key="areaId" ref="menuListTree" show-checkbox v-if="visible">
						</el-tree>
					</el-form-item>
				</el-scrollbar>
			</el-form>
...
<script>
	...
	import { areaList } from '@/api/transport';
			...
			init(rowIndex, cityList, allSelectCityList, type) {
				...
				if(this.menuList.length === 0) {
					areaList().then(response => {
						this.menuList = treeDataTranslate(response.data, 'areaId', 'parentId')
