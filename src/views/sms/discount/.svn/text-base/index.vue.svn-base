	...
	<div class="app-container">
		...
		<div class="table-container">
			<el-table ref="homeAdvertiseTable" :data="list" style="width: 100%;" @selection-change="handleSelectionChange" v-loading="listLoading" border>
				...
				<el-table-column label="订单实付金额"  align="center">
					<template slot-scope="scope">{{scope.row.discountOrderMoney}}</template>
				</el-table-column>
				<el-table-column label="付款订单数"  align="center">
					<template slot-scope="scope">{{scope.row.delegationDiscountNumber}}</template>
				</el-table-column>
				<el-table-column label="付款人数"  align="center">
					<template slot-scope="scope">{{scope.row.delegationMemberNumber}}</template>
				</el-table-column>
				<el-table-column label="关闭/开启" align="center">
					<template slot-scope="scope">
				...
				<el-table-column label="操作" align="center">
					<template slot-scope="scope">
						...
						<el-button size="mini" type="text" @click="handleUpdate(scope.$index, scope.row)">失效
						</el-button>
		...
		<el-dialog title="效果数据" :visible.sync="dialogTableVisible">
			...
			<el-row style="padding-top: 20px;">
				...
				<el-col :span="6">
					<div class="grid-content bg-purple-light">￥{{assebleEffect.countMoney==null?0:assebleEffect.countMoney-assebleEffect.totalAmount==null?0:assebleEffect.totalAmount}}</div>
				</el-col>
				<el-col :span="6">
					<div class="grid-content bg-purple">{{parseFloat((assebleEffect.countMoney-assebleEffect.totalAmount)/assebleEffect.countMoney).toFixed(2)|formatNan}}</div>
				</el-col>
				<el-col :span="6">
					<div class="grid-content bg-purple">{{assebleEffect.countCouponAmount==null?0:assebleEffect.countCouponAmount}}</div>
				</el-col>
			</el-row>
			<el-row style="padding-top: 20px;">
				<el-col :span="6">
...
<script>
	import { fetchList, deleteHomeAdvertise, EffectList, updateStatus } from '@/api/discount';
...
//				},
		...
		methods: {
			...
			handleAdd() {
				this.$router.push({
					path: '/sms/addDiscount'
				})
			},
			handleUpdate(index, row) {
				this.$router.push({
					path: '/sms/updateDiscount',
					query: {
						id: row.id
					}
				})
