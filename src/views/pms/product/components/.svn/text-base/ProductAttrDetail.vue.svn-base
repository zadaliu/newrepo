  ...
  <div style="margin-top: 50px">
    <el-form :model="value" ref="productAttrForm" label-width="120px" style="width: 720px" size="small">
      ...
      <el-form-item style="text-align: center">
        ...
        <el-button type="primary" size="medium" @click="handleNext">下一步，填写基础分销</el-button>
      </el-form-item>
    </el-form>
  </div>
</template>
      ...
      mergeProductAttrPics() {
        for (let i = 0; i < this.selectProductAttrPics.length; i++) {
          for (let j = 0; j < this.value.skuStockList.length; j++) {
          	console.log("______________"+this.value.skuStockList.length);
           console.log(this.selectProductAttrPics);
//          if (this.value.skuStockList[j].sp1 === this.selectProductAttrPics[i].name) {
              this.value.skuStockList[j].pic = this.selectProductAttrPics[j].pic;
